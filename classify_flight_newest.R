...
for (model_name in model_list) {
  for (ix1 in 1:length(data_fr_list$test_data)) {
    ...
    train_new_data <- data_fr_list$train_data[ix1]
    names(train_new_data) <- names(data_fr_list$train_data)[ix1]
    test_new_data <- data_fr_list$test_data[ix1]
    names(test_new_data) <- names(data_fr_list$test_data)[ix1]

    print(paste(ix1, length(train_new_data)))

    model_used_list <- model_use(
      model_name, train_new_data,
      test_new_data, data_fr_list$train_label,
      data_fr_list$test_label,
      tree_name = paste("trees_new_one/all", ix1, "tree.pdf", sep = "_")
    )

    print(paste("trees_new_one/all", ix1, "tree.pdf", sep = "_"))
    colname_model <- paste(model_name, ix1, sep = "_")
    if (model_name == "k-NN") {
    ...
    write.csv(df_predictions_train, paste("trees_new_one/predictions_train", model_name, ix1, "new.csv", sep = "_"), row.names = FALSE)
    write.csv(df_predictions_test, paste("trees_new_one/predictions_test", model_name, ix1, "new.csv", sep = "_"), row.names = FALSE)

    print(paste("trees_new_one/predictions_train", model_name, ix1, "new.csv", sep = "_"))
    print(paste("trees_new_one/predictions_test", model_name, ix1, "new.csv", sep = "_"))
