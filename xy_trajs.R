...
plot_2D <- function(first_dim, second_dim) { 
  ...
  for (filename_for_traj in filenames_for_trajs) {
    ...
    legend(poslegend, legend = c("Original", "Glatko"), col = c("blue", "red"), lty = c(2, 1), lwd = c(1, 2), text.width = strwidth("Originalnost"))
      
    # Spremanje dijagrama
    
    dev.copy(png, filename = paste(dir_for_plot, gsub("csv", "png", gsub("weather", paste(first_dim, second_dim, sep = "_"), filename_for_traj)), sep = "//"))
    dev.off()
