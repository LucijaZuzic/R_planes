...
model_list <- c("k-NN", "Linear SVM", "RBF SVM", "Decision Tree", "Random Forest", "Naive Bayes", "Multilayer Perceptron", "AdaBoost", "Quadratic Discriminant Analysis")
...
for (i in 2:ncol(data_fr)) {
  df_new_feat <- filter(df_new, df_new[, 1] == names(data_fr)[i])
  print(df_new_feat) 
}

for (model_name in model_list) { 
  df_ml_feat <- filter(df_new, df_new[, 3] == model_name)
  print(df_ml_feat) 
}
